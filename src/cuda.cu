...
void cuda_preprocess(uint8_t* src, int src_width, int src_height, float* dst, int dst_width, int dst_height, cudaStream_t stream){
    ...
    float scale = min(dst_height / (float)src_height, dst_width / (float)src_width);
    s2d.value[0] = scale; 
    s2d.value[1] = 0;
    ...
    Mat m2x3_s2d(2, 3, CV_32F, s2d.value);
    Mat m2x3_d2s(2, 3, CV_32F, d2s.value);
    invertAffineTransform(m2x3_s2d, m2x3_d2s);
